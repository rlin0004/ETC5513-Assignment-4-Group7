...
gender_rate %>%
   filter(sex != "both",
          offence_type != "Total") %>%
  group_by(offence_type, sex) %>%
  summarise(mean_rate = mean(count, na.rm = TRUE)) %>%
  ggplot(aes(y = offence_type, x = mean_rate, color = sex), position = position_dodge2()) +
  geom_col()
```
...
gender %>%
  ...
  group_by(year, offence_type, sex) %>%
  summarise(mean_year = mean(count, na.rm = TRUE)) %>%
  ggplot(aes(x = offence_type, y = mean_year, fill = sex)) +
